...
def train_thread(args, tpu, id, q):
    ...
    latest_ckpt = latest_model_index(json.loads(open(f"configs/{args.model}.json").read())["model_path"])
...
def main(_run):
    ...
    os.system("screen -S tensorboard_{} -d -m bash -c '. env/bin/activate; tensorboard --logdir {} --port {} --bind_all --reload_multifile=true || tensorboard --logdir {} --port {} --reload_multifile=true'".format(_run._id, params["model_path"], tensorboard_port,params["model_path"], tensorboard_port,))
